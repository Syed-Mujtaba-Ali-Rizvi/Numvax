...
import { CalculatorRelatedTools, RelatedTool } from '../calculator/CalculatorRelatedTools';
import { HeaderSearch } from '../navigation/HeaderSearch';
import { BrowseToolsButton } from '../navigation/BrowseToolsButton';
import { AdSlot } from '../ads/AdSlot';
import { JsonLd } from '../seo/JsonLd';
import { CheckCircle2, ShieldCheck, HelpCircle, FileQuestion, BookOpen, Layers, Sparkles } from 'lucide-react';
...
export const ToolLayout: React.FC<ToolLayoutProps> = async ({
  slug,
  title,
  subtitle,
  categoryName,
  ...
  const finalInstructionsTitle = instructionsTitle || t('howToUse');
  const finalTrustCopy = trustCopy || t('trustCopy');
  ...
  const breadcrumbs = [
    ...
    { name: tBreadcrumb('tools'), url: locale === 'en' ? `${baseUrl}/tools` : `${baseUrl}/${locale}/tools` },
    { name: categoryName, url: locale === 'en' ? `${baseUrl}/tools/category/${categorySlug}` : `${baseUrl}/${locale}/tools/category/${categorySlug}` },
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 py-5 sm:py-7 flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2.5 mt-1">
        ...
        {/* Privacy Trust Badge */}
        {finalTrustCopy && (
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#FAF8F5] border border-neutral-200 text-neutral-700 text-xs font-medium w-fit mt-1 shadow-2xs">
            <ShieldCheck className="w-4 h-4 text-emerald-700" />
            <span>{finalTrustCopy}</span>
          </div>
        )}
      </div>
      ...
      {/* Structured SEO Content Section (Server-Rendered HTML) */}
      <div className="w-full flex flex-col gap-6 mt-6">
        ...
        {instructionsDescription && (
          <section className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-xs leading-relaxed">
            <h2 className="text-xl font-bold text-neutral-900 mb-3 flex items-center gap-2">
              ...
              {finalInstructionsTitle}
            </h2>
            <div className="p-4 sm:p-5 bg-neutral-50 rounded-xl text-sm text-neutral-800 border border-neutral-200 whitespace-pre-line leading-relaxed font-normal">
              {instructionsDescription}
            </div>
          </section>
        )}
        ...
        {/* Related & Contextual Tools Cross-Links */}
